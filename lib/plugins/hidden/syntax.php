...
class syntax_plugin_hidden extends DokuWiki_Syntax_Plugin {
  ...
  function connectTo($mode) {
    ...
    $this->Lexer->addEntryPattern('<--->(?=.*?</--->)', $mode,'plugin_hidden'); // HACK: Testing modifications on open tag
    $this->Lexer->addSpecialPattern('<hiddenSwitch[^>]*>', $mode,'plugin_hidden');
     }
  function postConnect() {
    $this->Lexer->addExitPattern('</hidden>','plugin_hidden');
    $this->Lexer->addExitPattern('</--->','plugin_hidden'); // HACK: Testing modifications on close tag
  }
    ...
    if($mode == 'xhtml'){
      switch ($data['state']) {
        case DOKU_LEXER_ENTER :
          ...
          $renderer->doc .= '<div class="hiddenBody"><pre>'."\n"; // HACK: open hiddenBody and hiddenGlobal
          break;
        ...
        case DOKU_LEXER_EXIT :
          $renderer->doc .= ""."\n</pre>\n</div></div>"; // HACK: close hiddenBody and hiddenGlobal
